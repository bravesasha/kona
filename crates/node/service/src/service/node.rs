...
use crate::{
    ...
    SequencerConfig,
    actors::{
        BlockStream, DerivationInboundChannels, EngineInboundData, L1QueryActorBuilder,
        L1WatcherActorBuilder, NetworkInboundData, QueuedUnsafePayloadGossipClient,
...
impl RollupNode {
    ...
    pub async fn start(&self) -> Result<(), String> {
        ...
        // Create the two L1 actors
        let head_stream = BlockStream::new(
            self.l1_provider.clone(),
            BlockNumberOrTag::Latest,
            Duration::from_secs(4),
        )?;
        let finalized_stream = BlockStream::new(
            self.l1_provider.clone(),
        ...
        // Create the [`L1QueryActor`]. Previously a subactor to `l1_watcher`.
        let l1_query = L1QueryActorBuilder::new()
            .with_rollup_config(self.config.clone())
        ...
        // Create the [`L1WatcherRpcActor`]. Previously known as the DA watcher actor.
        let l1_watcher = L1WatcherActorBuilder::new()
            .with_rollup_config(self.config.clone())
            .with_l1_provider(self.l1_provider.clone())
